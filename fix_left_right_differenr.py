...
def parse_points(points):
    """ Parse a string representation of points into a NumPy array. """
    try:
        # Assuming the points are stored as a string of list format
        return np.array(eval(points))
    except:
        return np.empty((0, 2))

def remove_middle_points(points, target_num):
    """ Remove points from the middle to reduce to exactly target_num. """
    num_to_remove = len(points) - target_num
    if num_to_remove <= 0:
        return points  # If already less than target, return original
    start_idx = len(points) // 2 - num_to_remove // 2
    end_idx = start_idx + num_to_remove
    return np.delete(points, slice(start_idx, end_idx), axis=0)

def linear_interpolate(points, target_num):
    """ Linearly interpolate to adjust the number of points. """
    if len(points) < 2:
...
def enhanced_process_data(row):
    ...
    # Determine the number of points to adjust based on specific criteria
    if len(left_vertices) < 6 or len(right_vertices) < 6:
        target_num = 2  # Target number of points if less than 6
    else:
        target_num = max(6, len(left_vertices), len(right_vertices))  # At least 6 points or as many as the larger side

    left_adjusted = remove_middle_points(left_vertices, target_num)
    right_adjusted = remove_middle_points(right_vertices, target_num)

    # If after removal points are still more than target, interpolate to match the target
    left_final = linear_interpolate(left_adjusted, target_num)
    right_final = linear_interpolate(right_adjusted, target_num)

    return left_final.tolist(), right_final.tolist()

# Assuming df is the DataFrame loaded from the CSV
#df = pd.read_csv('path_to_your_csv_file.csv')  # Update the path to your CSV file
df[['NewLeftPoints', 'NewRightPoints']] = df.apply(enhanced_process_data, axis=1, result_type='expand')
output_path_enhanced = './data/enhanced_processed_lanelet_points.csv'  # Update the path to your output CSV file
df.to_csv(output_path_enhanced, index=False)
...
for index, row in df.iterrows():
    ...
    if lanelet is not None:
        ...
        for point in row['NewRightPoints']:
            ...
            ET.SubElement(new_point, 'y').text = str(point[1])
# 将修改后的XML树写入文件
tree.write('./data/yizhuangfix.xml')
